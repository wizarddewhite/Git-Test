...
func CreateNode() {
	...
	client := vultr.NewClient("S4S4MTM6UQV4RMKWRM4DPM3NSROV3E5R35BA", nil)
	server, err = client.CreateServer("test", 7, 201, 241, nil)
	if err.Error() == "Plan is not available in the selected datacenter.  This could mean you have chosen the wrong plan (for example, a storage plan in a location that does not offer them), or the location you have selected does not have any more capacity." {
		server, err = client.CreateServer("test", 19, 201, 241, nil)
	}
