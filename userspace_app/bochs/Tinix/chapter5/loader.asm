...
.MemChkLoop:
	...
	int	15h			; int 15h, 成功时将把ARDS填充到es:di指定的内存空间
	jc	.MemChkFail
	add	di, 20			; 移动es:di到下一个内存位置
...
LABEL_FILENAME_FOUND:			; 找到 KERNEL.BIN 后便来到这里继续
	...
	add	cx, ax
	add	cx, DeltaSectorNo	; 这时 cl 里面是 KERNEL.BIN 的起始扇区号 (从 0 开始数的序号)
	mov	ax, BaseOfKernelFile
	mov	es, ax			; es <- BaseOfKernelFile
	mov	bx, OffsetOfKernelFile	; bx <- OffsetOfKernelFile	于是, es:bx = BaseOfKernelFile:OffsetOfKernelFile = BaseOfKernelFile * 10h + OffsetOfKernelFile
...
LABEL_PM_START:
	...
	;              ┣━━━━━━━━━━━━━━━━━━┫
	;              ┃■■■■■■■■■■■■■■■■■■┃
	;       90000h ┃■■■■■■■LOADER.BIN■■■■■■┃ somewhere in LOADER ← esp, GDT
	;              ┣━━━━━━━━━━━━━━━━━━┫
	;              ┃■■■■■■■■■■■■■■■■■■┃
	;       80000h ┃■■■■■■■KERNEL.BIN■■■■■■┃
	;              ┣━━━━━━━━━━━━━━━━━━┫
...
InitKernel:	; 遍历每一个 Program Header，根据 Program Header 中的信息来确定把什么放进内存，放到什么位置，以及放多少。
	...
	add	esi, BaseOfKernelFilePhyAddr		; esi <- BaseOfKernel + pELFHdr->e_phoff 指向第一个Program Header
.Begin:
	mov	eax, [esi + 0]			; 判断Program Header类型
	cmp	eax, 0				; PT_NULL
	jz	.NoAction
	...
	add	esp, 12				;	┛
.NoAction:
	add	esi, 020h			; esi += pELFHdr->e_phentsize指向下一个Program Header
	dec	ecx
	jnz	.Begin
